/*
 =======================================================================================================================
	...
	File:		fn_objectsMapper.sqf
	Author:		Joris-Jan van 't Land, T-800a
...

{
	...
	if ((random 1) > _rdm) then 
	{
		...
		_newObj = _type createVehicle [0,0,0];
		...
		if ( _type in [	"Land_HBarrier_5_F", "Land_CncBarrier_stripes_F", "Land_HBarrierBig_F", "Land_HBarrier_Big_F", "Land_HBarrier_3_F", "Land_HBarrier_1_F", "Land_BagFence_Round_F",
						"Land_BagFence_Corner_F", "Land_BagFence_Long_F", "Land_BagFence_Short_F", "Land_Razorwire_F" ]) then
		{
			_newObj setVectorUp ( surfaceNormal( getPos _newObj ));
		};

		if ( _newObj isKindOf "LandVehicle" ) then { _newObj setVehicleLock "LOCKEDPLAYER"; };
		if ( _type in [ "Land_Cargo20_military_green_F", "Land_d_Stone_HouseSmall_V1_F", "Land_d_Windmill01_F" ] ) then { _newObj allowDamage false; };
...
// add to cleanup
T8SME_server_var_arrayCleanup pushBack _newObjs;

...

{
	...
	_missionVehicle = __GetMVAR( _vehicle, objNull );
...
//	__DEBUG( __FILE__, "CHECK _nearBase", _nearBase );
	...
	if ( _respawn ) then
	{
		...
		if ( !( getMarkerPos _spawn isEqualTo [0,0,0]) AND { count (( getMarkerPos _spawn ) nearObjects [ "All", 7 ] ) < 1 }) then
		{
			...
			if ( _mobileRespawn ) then { _displayName = format [ "%1 (Mobile HQ)", _displayName ]; };
			private _hint = format [ ( localize "STR_SMEGen_hint_vehicleRespawn" ), _displayName ];	
